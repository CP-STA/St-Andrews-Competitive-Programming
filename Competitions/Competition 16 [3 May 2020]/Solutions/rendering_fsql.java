...
public class rendering_fsql {
    static TreeMap<Integer, stu> students;
    static ArrayList<Integer> stu_list = new ArrayList<>();
    static String[] qu;
    ...
    public static void solver(String[] queries) throws IOException {
    for (String s:queries)
    {
        if (s == null)
            continue;
        String sp1[] = s.split("::");
        if (sp1[0].equals("ADDROW"))
        {
            String[] params = sp1[1].split(":");
            addrow(Integer.parseInt(params[0]), params[1], Integer.parseInt(params[2]));
   //         System.out.println("NULL");
            fw.write("NULL");
            fw.write('\n');
        }
        else if (sp1[0].equals("REMOVE"))
        {
            removerow(Integer.parseInt(sp1[1]));
    //       System.out.println("NULL");
            fw.write("NULL");
            fw.write('\n');
        }
        else if (sp1[0].equals("QUERY"))
        {
            query(Integer.parseInt(sp1[1]));
            //       System.out.println("NULL");
        }
        else
        {
    ...
    public static void addrow(int stu_id, String stu_name, int stu_mark)
    {
        stu s = new stu();
        s.student_name = stu_name;
        s.student_mark = stu_mark;
        // make sure no such id exists
        students.putIfAbsent(stu_id, s);
    }
    public static void removerow (int stu_id)
    {
        students.remove(stu_id);
    }
    public static void select () throws IOException {
        for (int stu_ids:students.keySet())
        {
            stu s = students.get(stu_ids);
       //     System.out.println (stu_ids + "  " + s.student_name + "  " + s.student_mark);
            fw.write(stu_ids + "  " + s.student_name + "  " + s.student_mark);
            fw.write('\n');
        }
    }
    public static void query(int stu_id) throws IOException{
        stu s = students.get(stu_id);
        if (s == null)
        {
            // does not exist
        ...
        {
            fw.write(stu_id + "  " + s.student_name + "  " + s.student_mark);
        }
        fw.write('\n');
    }
    public static void testcase_generator() throws IOException {
        // randomly decide to add, remove or select a row
        Random rand = new Random();
        int seed = rand.nextInt(120) % 4;
        if (seed == 0)
        {
            // add a row
            StringBuilder sb = new StringBuilder();
            int stu_id = rand.nextInt(10001);
            ...
            String stu_name = names[rand.nextInt(names.length)];
            int stu_mark = rand.nextInt(101);
            sb.append("ADDROW::");
            sb.append(stu_id);
            sb.append(":");
            sb.append(stu_name);
        ...
        else if (seed == 1)
        {
            ...
            fw.write("SELECT::ALL");
            fw.write('\n');
            qu[qu_num] = "SELECT::ALL";
        }
        else if (seed == 2)
        {
            // query something
            if (stu_list.size() != 0) {
                int rem_id = rand.nextInt(stu_list.size());
                fw.write("QUERY::" + stu_list.get(rem_id));
                fw.write('\n');
                qu[qu_num] = "QUERY::" + stu_list.get(rem_id);
            }
            else
            {
                    int rem_id = rand.nextInt(100);
                    fw.write("QUERY::" + rem_id);
                    fw.write('\n');
                    qu[qu_num] = "QUERY::" + rem_id;
            }
        }
        else
        {
            // remove something
                ...
                {
                    int rem_id = rand.nextInt(100);
                    fw.write("REMOVE::" + rem_id);
                    fw.write('\n');
                    qu[qu_num] = "REMOVE::" + rem_id;
                }
                else {
                    int rem_id = rand.nextInt(stu_list.size());
                    fw.write("REMOVE::" + stu_list.get(rem_id));
                    fw.write('\n');
                    qu[qu_num] = "REMOVE::" + stu_list.get(rem_id);
                }
       //     stu_list.remove(rem_id);
        }
    }
    public static void main(String[] args) throws IOException {
        for (int j = 0; j < 40; j++) {
            ...
            input_name.append("C:\\Users\\vishn\\OneDrive\\Documents\\IntelliJ\\competitivestandrews\\src\\rendering_fsql_data\\input\\input");
            if (j < 10)
                input_name.append("0");
            input_name.append(j);
            input_name.append(".txt");
            fw = new FileWriter(input_name.toString());
            int n_rows = new Random().nextInt(6000);
            qu = new String[n_rows];
            qu_num = 0;
            fw.write(Integer.toString(n_rows));
            fw.write('\n');
            ...
            output_name.append("C:\\Users\\vishn\\OneDrive\\Documents\\IntelliJ\\competitivestandrews\\src\\rendering_fsql_data\\output\\output");
            if (j < 10)
                output_name.append("0");
            output_name.append(j);
