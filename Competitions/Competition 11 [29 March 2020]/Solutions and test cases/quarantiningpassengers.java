...
class Pair
{
    // coordinate pair (x,y) that represents the location of a person
    int x, y;
    Pair(int x, int y)
    {
        this.x = x;
...
public class quarantiningpassengers {
    static String[][] passengers;
    static ArrayList<String>positivecases;
    ...
    static class ArrayGenerator{
        // class for generating the core array in this question
        // source for names: https://gist.github.com/ruanbekker/a1506f06aa1df06c5a9501cb393626ea
        ...
        {
            passengers = new String[7][N];
        }
        public void getName() {
            // generate names by picking two random names from the list and adding a "_" between them
            ...
            {
                // the seat should be empty as we have a conflict (this should be rare)
                passengers[counter%7][counter/7] = "empty";
            ...
            else
            {
                ...
                {
                    // aisle
                    passengers[counter%7][counter/7] = "aisle";
                }
                else
                {
                    namelist.add(sb.toString());
                    // not an aisle, put the passenger name
                    passengers[counter%7][counter/7] = sb.toString();
                }
            }
            // next passenger please
            counter++;
        }
        public void padder(int num_passengers)
        {
            // numbers of rows is known
            // pad the remaining seatas as empty
            for (int i = num_passengers; i < 7*passengers[0].length; i++)
            {
                if (i % 7 != 3)
                passengers[i%7][i/7] = "empty";
                else
                passengers[i%7][i/7] = "aisle";
            }
        }
        public void getPositive()
        {
            // get the list of passengers that are diagonised as positive
            Random rand = new Random();
            positivecases = new ArrayList<>();
            int num_positive = rand.nextInt(namelist.size());
            for (int i = 0; i < num_positive; i++)
            {
                String pos_case = namelist.get(rand.nextInt(namelist.size()));
                if (positivecases.contains(pos_case))
                {
                    // duplicate, nope
    ...
    {
        // test case data must have already been generated by now
        // precompute the data to a hashmap
        ...
        for (int i = 0; i < 7; i++)
        {
            ...
            {
                if (passengers[i][j].equals("empty") || passengers[i][j].equals("aisle"))
                {
                    // don't add
                }
                else
                {
                    Pair P = new Pair(i, j);
                    core.put(passengers[i][j], P); // add into hashmap
                }
            }
        }
        ...
        for (String s:positivecases)
        {
            ...
            if (P.x != 0)
            {
                ...
                if (!passengers[P.x - 1][P.y].equals("empty") && !passengers[P.x - 1][P.y].equals("aisle"))
                {
                    ans.add(passengers[P.x - 1][P.y]);
                }
            }
            if (P.x < passengers.length - 1)
            {
            ...
            if (P.y != 0)
            {
                ...
                if (!passengers[P.x][P.y - 1].equals("empty"))
                ans.add(passengers[P.x][P.y - 1]);
            }
            if (P.y < passengers[0].length - 1)
            {
                // downward checking allowed
                if (!passengers[P.x][P.y + 1].equals("empty"))
                ans.add(passengers[P.x][P.y + 1]);
            }
            // the passengers self-quarantining should also be affected
            ans.add(s);
        }
    }
    public static void writeInput(int j) throws IOException
    {
        int N = passengers[0].length;
        StringBuilder input_name = new StringBuilder();
        input_name.append("C:\\Users\\vishn\\OneDrive\\Documents\\IntelliJ\\competitivestandrewschallenge2\\src\\quarantining_data\\input\\input");
        if (j < 10)
            input_name.append("0");
        input_name.append(j);
        input_name.append(".txt");
        FileWriter fw = new FileWriter(input_name.toString());
        fw.write(Integer.toString(N)); // number of rows
        fw.write('\n');
        // write all the rows
        for (int i = 0; i < N; i++)
        {
            for (int k = 0; k < 7; k++)
            {
                fw.write(passengers[k][i]);
                if (k != 6)
                    fw.write(" ");
            }
            fw.write('\n');
        }
        fw.write(Integer.toString(positivecases.size())); // number of positive cases
        fw.write('\n');
        // list of positive passengers
        for (int t = 0; t < positivecases.size(); t++)
        {
            fw.write(positivecases.get(t));
            if (t != positivecases.size() - 1)
                fw.write('\n');
        }
        fw.close();
    }
    public static void writeOutput(int j) throws IOException
    {
        StringBuilder input_name = new StringBuilder();
        input_name.append("C:\\Users\\vishn\\OneDrive\\Documents\\IntelliJ\\competitivestandrewschallenge2\\src\\quarantining_data\\output\\output");
        if (j < 10)
            input_name.append("0");
        input_name.append(j);
        input_name.append(".txt");
    ...
    public static void main(String[] args) throws IOException {
        // first generate the arrays for the question
        // generate 50 test cases
        for (int j = 0; j < 50; j++) {
            namelist = new ArrayList<>();
            ans = new TreeSet<>();
            Random rand = new Random();
            int N = rand.nextInt(25000); // number of rows
            int num_passengers = rand.nextInt(N*7); // number of passengers
            ArrayGenerator A;
            A = new ArrayGenerator(N);
            for (int i = 1; i <= num_passengers; i++) {
                A.getName(); // add it in the core array
            }
            A.padder(num_passengers); // pad the remaining (2N - num_passengers) with empty
            A.getPositive(); // get the positive candidates
            // now use that to generate the input file
            writeInput(j);
            // ------- test case data generated, now we need to solve the problem with the provided data -----
            solver();
